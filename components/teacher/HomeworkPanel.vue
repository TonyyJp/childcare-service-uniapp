<template>
  <view class="tab-page">
    <view class="gradient-header" style="background:linear-gradient(135deg,#FF7043 0%,#FF8A65 100%);">
      <view class="safe-nav-bar" style="display:flex;align-items:center;justify-content:space-between;padding-bottom:16rpx;">
        <view>
          <text style="font-size:44rpx;font-weight:800;color:white;display:block;">作业中心</text>
          <text style="font-size:24rpx;color:rgba(255,255,255,0.8);">{{ primaryClassName }} · {{ homeworkList.length }} 份</text>
        </view>
        <view style="background:rgba(255,255,255,0.25);border-radius:20rpx;padding:16rpx 28rpx;" @click="openAddHw">
          <text style="color:white;font-size:26rpx;font-weight:700;">+ 布置</text>
        </view>
      </view>
      ...
      <view style="padding:24rpx 40rpx;">
        ...
        <view v-else-if="hwTab === '作业登记'">
          <view v-if="!homeworkList.length" style="padding:48rpx 0;text-align:center;">
            <text style="font-size:26rpx;color:#8D6E63;">暂无作业，点右上角布置</text>
          </view>
        ...
        <view v-else-if="hwTab === '作业反馈'">
          ...
          <view v-else>
            ...
            <view v-for="f in feedbacks" :key="f.studentId" style="display:flex;align-items:flex-start;gap:20rpx;padding:24rpx 0;border-bottom:1rpx solid #F5F0EC;" @click="openReview(f)">
    ...
    <view v-if="showReview" class="overlay" @click="showReview = false">
      <view class="sheet" @click.stop>
        <view class="sheet-handle" />
        <text class="sheet-title">批改 · {{ reviewTarget?.name || '' }}</text>
        <view style="display:flex;flex-direction:column;gap:24rpx;">
          <view>
          ...
          <view>
            ...
            <textarea class="form-input" style="height:140rpx;padding-top:16rpx;" placeholder="给家长的反馈…" v-model="reviewForm.text" />
          </view>
          <view class="primary-btn" :style="{ opacity: reviewBusy ? 0.6 : 1 }" @click="submitReview">
            <text style="color:white;font-size:30rpx;font-weight:800;">{{ reviewBusy ? '提交中…' : '确认批改' }}</text>
          </view>
        </view>
      </view>
    </view>
...
async function loadHomeworks() {
  ...
  try {
    const data = await fetchHomeworks({ status: 'published', perPage: 50 })
...
async function loadHwSubmissions(homeworkId) {
  try {
    ...
    feedbacks.value = (data?.list || []).map((row, i) => {
      const statusLabel = { unsubmitted: '未提交', submitted: '待批改', graded: '已批改' }[row.status] || row.status
      return {
        ...
        hint: row.status === 'unsubmitted' ? '尚未提交' : (row.status === 'graded' ? '已批改，可再次修改评级' : '已提交，点按批改'),
...
async function submitReview() {
  if (reviewBusy.value || !reviewTarget.value?.submissionId) return
